#!/usr/bin/env python3
"""
build_player.py  v2.3a  (2025-12-13)
...
Provenance:
  ...
  - v2.3: inline story.json into __STORY_JSON__ in the template when present, and
          normalize FAQ_Loc / Quiz_Loc to avoid broken relative links.
  - v2.3a: fix adjust_image_width replacement string syntax.
...
Purpose:
  ...
  - Produce a SOP-specific *_player.html with:
      ...
      * (For old templates) The default story.json path in the helper input wired
        to the SOP's story.
      ...
      * For new templates with __STORY_JSON__, the story.json contents are
        inlined so the player can run without a network fetch.
    ...
    def _write(self, level: str, msg: str) -> None:
        ...
        if self.logfile is not None:
            try:
                ...
                print(
                    f"[WARN] Could not write to log file {self.logfile}: {e}",
                    file=sys.stderr,
                )
...
def normalize_faq_quiz_paths(story: Dict[str, Any], log: Logger) -> None:
    """
    Normalise FAQ_Loc / Quiz_Loc so that:
      - If they start with '/' or 'http', they are left as-is.
      - If they are bare relative paths like 'outputs/faq', they are turned into
        '/outputs/faq' so links don't accidentally become
        'outputs/players/outputs/faq/...'.
    """
    frames = story.get("frames", [])
    for frame in frames:
        code = frame.get("frame_code", "UNKNOWN")
        for key in ("FAQ_Loc", "Quiz_Loc"):
            val = frame.get(key)
            ...
            if normalized != raw:
                log.info(f"Normalized {key} for frame {code}: '{raw}' -> '{normalized}'")
...
def fs_to_web_story_path(story_fs: Path, out_fs: Path, log: Logger) -> str:
    """
    Compute a path that the browser can use to fetch story.json for older
    templates that still use a manual Load button / fetch() approach.

    For SOP_Build we typically serve from the /outputs folder in dev, with URLs:

      /outputs/players/LineEnt2_player.html
      /outputs/story/LineEnt2/story.json

    So the natural relative path from the player to the story is:
...
def get_default_template(log: Logger) -> Path:
    ...
    template = here.parent / "templates" / "sop_player.html"
...
def adjust_story_input(html: str, story_web_path: str, log: Logger) -> str:
    """
    For older templates with:
      <input type="text" id="story" ... value="...">
    set that default path to story_web_path.

    For your new inline-JSON template there is no input#story, so this
    function will just log a warning and leave the HTML unchanged.
    """
    pattern = re.compile(
    ...
    if pattern.search(html):
        ...
        log.info("No <input id=\"story\" ...> element found; skipping story path patch.")
    return html
...
def adjust_exit_link(html: str, exit_href: Optional[str], log: Logger) -> str:
    """
    If exit_href is provided, wire the Exit/Home button href.
...
def adjust_image_width(html: str, image_width: Optional[int], log: Logger) -> str:
    ...
    (or #imgbox img in older templates).
    """
    if image_width is None:
        log.info("No --image-width provided; leaving image width CSS as-is.")
        return html

    # Try newer '.imgbox img' style first
    pattern_new = re.compile(
        r"(\.imgbox\s+img\s*\{\s*width:\s*)\d+(\s*%;)",
        re.IGNORECASE,
    )
    # Older '#imgbox img { max-width: ... }' fallback
    pattern_old = re.compile(
        r"(#imgbox\s+img\s*\{\s*max-width:\s*)\d+(\s*%;\s*\})",
        re.IGNORECASE,
    )

    if pattern_new.search(html):
        replacement_new = rf"\g<1>{image_width}\g<2>"
        html = pattern_new.sub(replacement_new, html, count=1)
        log.info(f"Updated .imgbox img width to: {image_width}%")
    elif pattern_old.search(html):
        html = pattern_old.sub(rf"\g<1>{image_width}\g<2>", html, count=1)
        log.info(f"Updated #imgbox img max-width to: {image_width}%")
    else:
        log.warn("Could not find .imgbox/#imgbox image rule; no width changes made.")
    return html


def inline_story_json(html: str, story: Dict[str, Any], log: Logger) -> str:
    """
    If the template contains the magic marker __STORY_JSON__ (as in your new
    SOP_Build player template), replace it with the full JSON from story.json.
    """
    placeholder = "__STORY_JSON__"
    if placeholder not in html:
        log.info("Template does not contain __STORY_JSON__; skipping JSON inlining.")
        return html

    raw_json = json.dumps(story, ensure_ascii=False)
    # Safety: avoid closing the script tag from inside the JSON
    safe_json = raw_json.replace("</script", "<\\/script")

    html = html.replace(placeholder, safe_json)
    log.info("Inlined story.json into __STORY_JSON__ placeholder in template.")
    return html
...
def build_player(
    ...
    # Load & normalise story.json
    story = load_story(story_path, logger)
    normalize_faq_quiz_paths(story, logger)
    ...
    # Determine web story path (for older templates using input#story + fetch)
    ...
    # Patch HTML pieces
    html = inline_story_json(html, story, logger)
    ...
    # Ensure output directory & write file
...
def parse_args(argv: Optional[list] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Build a SOP-specific *_player.html from story.json "
            "using the SOP player template."
        )
    )
    parser.add_argument(
        "--story",
    ...
    parser.add_argument(
        "--out",
        required=True,
        help="Output HTML path for the SOP-specific player "
             "(e.g. outputs/players/LineEnt_player.html).",
    )
    parser.add_argument(
        "--title",
        required=False,
        default=None,
        help=(
            "Browser tab title. If omitted, will use "
            "'<Function> – <SubEntity> SOP Player – EdxBuild' if available."
        ),
    )
    parser.add_argument(
        "--mode",
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Slide image width in percent (e.g. 65). Adjusts image width in template.",
    )
    parser.add_argument(
        "--exit",
        dest="exit_href",
        ...
        default=None,
        help=(
            "Href for Exit button (e.g. index.html or /site/BUILD/index.html#some-anchor). "
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional path to a non-default sop_player.html template. "
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional log file path. If given, writes a simple text log there "
